...
- **Validated scoring computed automatically.** PHQ-9 total + severity band, GAD-7 anxiety category, AUDIT alcohol-use score, custom scoring rules — all computed during `quickq refresh` and queryable from `agg_respondent_scores` without writing the scoring logic in your analysis code.
